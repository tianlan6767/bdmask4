...
class prune_model(nn.Module):
	def __init__(self, model, factor_removed=0.75):
		...
		for name, m in self.modulelist:

			if m.__class__.__name__ == "Conv2d":
				channels_sum = torch.sum(torch.square(m.weight),dim=(1,2,3)).view(-1).tolist()
				# for layer in m.weight:
				# 	self.layer_eval_list.append(layer_eval(layer))
				self.layer_eval_list.extend(channels_sum)
		...
		def replace_inside(modules):
			...
			for name, m in modules.named_modules():
				if isinstance(m, (Conv2d,nn.Conv2d)):
					replace_conv2d.append(name)
					# setattr(modules, name, pruned_conv2d(m, self.cut_off))
			for name in replace_conv2d:
				setattr(modules, name, pruned_conv2d(eval(f"modules.{name}"), self.cut_off))
