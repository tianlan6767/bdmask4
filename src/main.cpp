...
int main(){
    string fcos_engine_path = R"(/media/ps/data/train/LQ/task/bdm/bdmask/workspace/models/JT/model-1016-batch10)";
    BdmApp bdmapp;
    ...
    string src = R"(/media/ps/data/train/LQ/task/bdm/bdmask/workspace/models/JT/imgs/imgs/3648/*.jpg)";
    string dst = R"(/media/ps/data/train/LQ/task/bdm/bdmask/workspace/models/JT/imgs/Tinf)";
    ...
    // warm-up
    for(int i = 0; i < 0; ++i){
        cv::Mat image(3648, 5472, CV_8UC3);
        image.setTo(128);
        fcos1->commit(image);
    }
    ...
    printf("共测试%d张, 多batch总耗时:%.4f秒, 图片平均耗时: %.4f毫秒\n", files.size() * noc_num, sum_mul/1000, (sum_mul / (files.size()*noc_num - batch_size)));
