...
namespace CUDAKernel{
	...
	*/
	...
	__global__ void warp_affine_bilinear_mask_kernel(uint8_t* src, int src_line_size, int src_width, int src_height, 
	uint8_t* dst, int dst_width, int dst_height,float* warp_affine_matrix_2_3, uint8_t const_value_st, int edge){
		int position = blockDim.x * blockIdx.x + threadIdx.x;
		if (position > edge) return;
	...
	void warp_affine_bilinear_mask(
		...
		cudaStream_t stream){
			...
			auto grid = CUDATools::grid_dims(jobs);
			auto block = CUDATools::block_dims(jobs);
			checkCudaKernel(warp_affine_bilinear_mask_kernel << <grid, block, 0, stream >> >(
						src, src_line_size, src_width, 
						src_height, dst, dst_width, dst_height,  matrix_2_3, const_value_st, jobs
						));
		}
